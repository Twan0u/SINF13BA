...
La programmation déclarative est un type de programmation qui ne contraint pas le programme dans son exécution mais spécifie seulement où il doit arriver.

Le **pattern Matching** est une manière de travailler avec une structure en fonction de celle-ci. Une forme plus évoluée de case of.

L'**efficacité temporelle est spatiale** décrit l'évolution de la consommation de mémoire ou le temps d'ecécution en fonction du changement sur la taille du problème.

La **programmation de haut degré** : programmation qui accepte une fonction comme argument d'une autre fonction.   

La **généricité** : utilisation de différentes fonctions placés en argument comme input de la même fonction (Ex: map)

### 2.6 Embedding
Les procédures peuvent être mises dans une structure de données, ce qui leur donne plusieurs usages :

* **Explicit lazy evaluation** (=exécution différée) : construit une petite partie de la structure et le reste en fonction des besoins à la volée.
